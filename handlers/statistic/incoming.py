...
from database import get_dirty_incoming_statistic, get_incoming_statistic
from filters.base_filters import UserExistsFilter
from lang.lang_based_provider import Lang, format_string, get_message
from lang_based_variable import MessageKey, DirtyIncomingStatistic, IncomingStatistic

router = Router(name="incoming_router")


@router.callback_query(DirtyIncomingStatistic.filter(), UserExistsFilter())
async def process_dirty_incoming_statistic(query: CallbackQuery, lang: Lang) -> None:
    await query.message.delete()
    statistic = list(await get_dirty_incoming_statistic())
    ...
    await query.message.answer_photo(photo=img_file,
                                     caption=format_string(
                                         get_message(MessageKey.USER_DIRTY_INCOMING_STATISTIC, lang),
...
@router.callback_query(IncomingStatistic.filter(), UserExistsFilter())
async def process_incoming_statistic(query: CallbackQuery, lang: Lang) -> None:
    await query.message.delete()
    statistic = list(await get_incoming_statistic())
    img = await generate_plot_async(statistic)
    text_table = generate_text_table(list(reversed(statistic)))
    img_file = BufferedInputFile(file=img.read(), filename=f"{uuid4()}.png")
    await query.message.answer_photo(photo=img_file,
                                     caption=format_string(
                                         get_message(MessageKey.USER_INCOMING_STATISTIC, lang),
                                         min_date=str(statistic[-1][0]),
                                         max_date=str(statistic[0][0]),
                                         text_table=text_table)
                                     )
...
def generate_plot(data) -> BytesIO:
    ...
    plt.title(f'Users Joined ({low_date}, {max_date})')
...
def generate_text_table(data) -> str:
    return tabulate(data[:10], headers=['date', 'user count'], tablefmt='grid')
