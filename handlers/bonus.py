...
from database import get_active_tasks_page, get_session, TaskType, get_active_task, Task, TaskDoneHistory, TransactionOperation
...
async def bonus_task_menu(query: CallbackQuery, callback_data: PaginationMove, lang: Lang, state: FSMContext) -> None:
    ...
    limit = 4
    s = get_session()
    pagination = get_active_tasks_page(s, query.from_user.id, page=callback_data.page, task_type=TaskType.BONUS, limit=limit)
...
async def select_bonus(query: CallbackQuery, callback_data: BonusTaskSelect, lang: Lang, state: FSMContext) -> None:
    await state.set_state(TaskStates.bonus_select)
    task: Task = get_active_task(get_session(), user_id=query.from_user.id, task_id=callback_data.task_id)
    if task is None:
...
async def process_bonus_task_done(query: CallbackQuery, callback_data: TaskDone, lang: Lang, state: FSMContext) -> None:
    s = get_session()
    s.begin()
    task: Task = get_active_task(get_session(), user_id=query.from_user.id, task_id=callback_data.task_id)
    if task is None:
    ...
    make_transaction_from_system(query.from_user.id, TransactionOperation.INCREMENT, task.done_reward, description="bonus task done",
