...
from lang_based_variable import Lang, MessageKey, MenuToSlotsCallback

router = Router(name="slots_router")


@router.callback_query(MenuToSlotsCallback.filter(), UserExistsFilter())
async def slots_menu_handler(query: CallbackQuery, bot: Bot, lang: Lang) -> None:
    await query.message.delete()
    await bot.send_photo(chat_id=query.message.chat.id,
                         photo=FSInputFile(path=os.getenv("PHOTO_04_PATH")),
