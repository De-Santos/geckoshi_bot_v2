...
from database import TransactionOperation, get_user_balance, SlotsBetHistory, BetType, with_session
...
@with_session
async def slots_play(query: CallbackQuery, callback_data: SlotsPlay, lang: Lang, state: FSMContext, s: AsyncSession = None) -> None:
    await state.set_state(SlotsStates.play)
    balance = await get_user_balance(query.from_user.id, s=s)
