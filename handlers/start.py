...
from keyboard_markup.inline_user_kb import get_start_user_kbm, get_require_subscription_kbm
...
from middleware.lang import LangProviderMiddleware
...
@router.message(UserExistsFilter(), StartStates.subscription)
async def require_subscription(message: types.Message, lang: Lang):
...
@router.callback_query(UserExistsFilter(), CheckStartMembershipCallback.filter(), StartStates.subscription)
async def check_subscription(query: CallbackQuery, callback_data: CheckStartMembershipCallback, state: FSMContext,
    ...
    refs = [kbs.get(i).get('ref') for i in range(1, 4)]
    r = [check_membership(query.from_user.id, link) for link in refs]
    if False in r:
        await require_subscription(query.message, callback_data.lang)
    else:
        await state.clear()
